...
void
set_lguest_trap(int irq, void *a)
{
	unsigned long lo, hi;
        /* A present interrupt gate. */
        unsigned int flags = 0x8e00;
...
trapinit0(void)
{
	...
	for(v = 0; v < 256; v++){
		...
		idt[v].d1 = d1;
		vaddr += 6;
		/* now tell LG ... the local copy is only a shadow */
//	iprint("Set vec %d to %p:%p\n", v, (void *) idt[v].d0, idt[v].d1);
...
//iprint("trap before splhi\n");
	splhi();

	/* delaysched set because we held a lock or because our quantum ended */
//iprint("up->delaysched %d clockintr %d\n", up->delaysched, clockintr);
	if(up && up->delaysched && clockintr){
		sched();
		splhi();
	}

//iprint("User %d proctl %d node %d\n", user, up->procctl, up->nnote);
	if(user){
