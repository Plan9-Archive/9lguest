...
/* this was NGDT but lguest demands GDT_ENTRIES */
Segdesc gdt[GDT_ENTRIES] =
{
[NULLSEG]	{ 0, 0},		/* null descriptor */
[KDSEG]		DATASEGM(1),		/* kernel data/stack */
[KESEG]		EXECSEGM(1),		/* kernel code */
[UDSEG]		DATASEGM(3),		/* user data/stack */
[UESEG]		EXECSEGM(3),		/* user code */
[TSSSEG]	TSSSEGM(0,1),		/* tss segment */
...
mmuinit0(void)
{
	u32 *pcr3, *pte;
	int i;
	/* do the bits here that were supposed to have been done in assembly */
	pcr3 = (u32 *) CPU0PDB;
...
/*	for(i = 0; i < 1024; i++)
		if (pcr3[i])
			iprint("mmuinit0 pdb[%#x] is %#x\n", i, pcr3[i]);*/
	/* don't do this just yet ... * /
	pte[(MACHADDR>>12)&0x3ff] = paddr((void *)CPU0MACH)|PTEWRITE|PTEVALID;
	  /**/
/*	for(i = 0; i < 1024; i++)
		if (pte[i])
			iprint("mmuinit0 pte[%#x] is %#x\n", i, pte[i]);*/
	memmove(m->gdt, gdt, sizeof gdt);
}

void
fixmach(){
	unsigned long pcr3 = getcr3();
	/* it's in the very first place */
	u32 *cr3 = (u32 *) (KZERO + pcr3);
	u32 *pmd, *pte;
...
mmuinit(void)
{
	...
	/* This is mostly meaningless at this point but let's see if it works */
...
//iprint("mmuswitch ...\n");
	if(proc->mmupdb){
		...
		/* no need to do an HCALL as it will be caught by the taskswitch */
		tmpunmap(pdb);
...
upallocpdb(void)
{
	...
	tmpunmap(pdb);
	up->mmupdb = page;
//iprint("Alloc'ed pdb, so set cr3 to it\n");
	putcr3(up->mmupdb->pa);
	splx(s);
}
...
putmmu(ulong va, ulong pa, Page*)
{
	...
	int lguestpdb = 0;
	u32 *pdb = m->pdb;
//iprint("putmmu, va %p, pa %p, pid %d, args %s\n", (void *)va, (void *)pa, 
//up->pid, up->args);
	/* simple test to see if we need to tell lguest. */
	if(up->mmupdb && (getcr3() == up->mmupdb->pa)){
//		iprint("putmmu: va %p pa %p tell LG\n", (void *)va, (void *)pa);
		lguestpdb = 1;
	}
...
//iprint("Setting page at &vpd[PDX(va)] %p\n", vpd[PDX(va)]);
		if (lguestpdb)
			lguest_set_pte_at((ulong)pdb, VPT+PDX(va)*BY2PG, 
				&vpd[PDX(va)], 
				PPN(page->pa)|PTEUSER|PTEWRITE|PTEVALID);
...
//iprint("old %#lx new %#lx\n", old, vpt[VPTX(va)]);
	if (lguestpdb)
		lguest_set_pte_at((ulong)pdb, va, 
			&vpt[VPTX(va)], pa|PTEUSER|PTEVALID);
...
iprint("vmapalloc fail or some such\n");
		iunlock(&vmaplock);
		return 0;
	}
	iunlock(&vmaplock);
...
{
	int have, i;
iprint("findhole a %p, n %d count %d\n", a, n, count);
	have = 0;
	for(i=0; i<n; i++){
		if(a[i] == 0)
			have++;
	...
	for(off=0; off<size; off+=pgsz){
		...
		if(pse && (pa+off)%(4*MB) == 0 && (va+off)%(4*MB) == 0 && (size-off) >= 4*MB){
			*table = (pa+off)|flag|PTESIZE|PTEVALID;
			lguest_set_pmd(table, (pa+off)|flag|PTESIZE|PTEVALID);
			pgsz = 4*MB;
		}else{
			...
			*pte = (pa+off)|flag|PTEVALID;
			 lguest_set_pte_at((ulong) pdb, 
						va+off, pte, (pa+off)|flag|PTEVALID);
			pgsz = BY2PG;
		}
	}
	return 0;
...
pdbunmap(ulong *pdb, ulong va, int size)
{
	ulong vae;
	ulong *table;
	int lguestpdb = (paddr(pdb) == getcr3());
	...
	while(va < vae){
		...
		if(!(table[PTX(va)] & PTEVALID))
			panic("vunmap: not mapped");
		if (lguestpdb)
			lguest_set_pte_at((ulong)pdb, va, &table[PTX(va)], 0);
		else
			table[PTX(va)] = 0;
		va += BY2PG;
	}
...
vmapsync(ulong va)
{
	...
	vpd[PDX(va)] = entry;
	lguest_set_pte(&vpd[PDX(va)], entry);
	/*
...
kmap(Page *page)
{
	...
	if(!(vpd[PDX(KMAP)]&PTEVALID)){
		...
		lguest_set_pte(&vpd[PDX(KMAP)], 
			up->kmaptable->pa|PTEWRITE|PTEVALID);
		flushpg((ulong)kpt);
		memset(kpt, 0, BY2PG);
		kpt[0] = page->pa|PTEWRITE|PTEVALID;
		lguest_set_pte(&kpt[0], page->pa|PTEWRITE|PTEVALID);
		up->lastkmap = 0;
		splx(s);
		return (KMap*)KMAP;
	}
	...
	for(i=0; i<NKPT; i++){
		if(kpt[(i+o)%NKPT] == 0){
			...
			lguest_set_pte(&kpt[o], page->pa|PTEWRITE|PTEVALID);
			up->lastkmap = o;
			splx(s);
...
tmpmap(Page *p)
{
	...
	*entry = p->pa|PTEWRITE|PTEVALID;
	lguest_set_pte(entry, p->pa|PTEWRITE|PTEVALID);
	flushpg(TMPADDR);
	return (void*)TMPADDR;
}
...
tmpunmap(void *v)
{
	...
	*entry = PPN(TMPADDR-KZERO)|PTEWRITE|PTEVALID;
	lguest_set_pte(entry, PPN(TMPADDR-KZERO)|PTEWRITE|PTEVALID);
	flushpg(TMPADDR);
}
