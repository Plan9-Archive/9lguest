 ...
 * mapped, so behave as if the mmu was not setup. Lguest is the same.
 */
TEXT _startKADDR(SB), $0
	MOVL	$_startPADDR(SB), AX
	/* test 1: br . */
	/*MOVL	$_startKADDR(SB), AX*/
	ANDL	$~KZERO, AX
	JMP*	AX
...
/*	Here begins the code for running under lguest. 
  *	A few things: memory is zero'd, we have gdt/paging,
  *	but not set up where we need it. 
  * 	all we need is a stack so we can call main()
  */
/* clone the low ptes to high virtual addresses */
	...
	/* test: can we exit? */
	/* test successful with 2.6.25-rc8 april 11 2008 */
	MOVL $LHCALL_CRASH, AX
	/*INT $LGUEST_TRAP_ENTRY*/

	/* oh, what fun. We need to get the struct entry for the pgdir. 
	 * we do the simple thing: set up an initialized pointer that points to it. 
	 */
	/* The Host put the toplevel pagetable in lguest_data.pgdir.  The movsl
	 * instruction uses %esi implicitly as the source for the copy we're
	 * about to do. */
	/* have to map virt to real, of course */
	/* p = &lguest_ptep; */
	MOVL $lguest_ptep - KZERO(SB), AX
	/* p = *p; */
	MOVL	(AX), AX
	/* now has virtaddr for pointer to pgdir */
	/* p &= ~KZERO; */
	ANDL	$~KZERO, AX
	/* p = *p; */
	MOVL	(AX), AX
	/* debug. This debug shows us we're right: 
	 *  Notify on address 0x7fff000
	 * which should be the top level, we think
	 *
	MOVL	AX,DX
	MOVL $LHCALL_NOTIFY, AX
	INT $LGUEST_TRAP_ENTRY
	 */
	/* now has physical pointer to the page table, we think */
	/* comment from linux: */
	...
	/* note that the pgdir is physical, so no need to adjust. */
	/* AX is the source */
	MOVL	AX, SI
	ADDL	$(((KZERO>>22)*4)&0xFFF), AX
	MOVL	AX, DI

	/* this also verifies: Notify on address 0x7fffc00
	MOVL	AX,DX
	MOVL $LHCALL_NOTIFY, AX
	INT $LGUEST_TRAP_ENTRY
	*/
...
/* the lguest iret. This is about 1e6 times simpler than Xen ... */
/* just tell the kernel not to interrupt you in the shower. */
TEXT lguest_iret(SB), $0
	PUSHL	AX
	MOVL	12(SP), AX
TEXT lguest_noirq_start(SB), $0
	/* rusty very wisely put irq_enabled as the first struct member */
	/* talk to jmk about how to do externs */
	MOVL	AX, lguest_data+0(SB)
	POPL	AX
	IRETL
TEXT lguest_noirq_end(SB), $0
...
TEXT	native_cpuid+0(SB),0,$0
	...
	/* here is the old code path that is not lguest */
	/* no need to mess with pa any more. 
	MOVL	$_startPADDR(SB), AX
	ANDL	$~KZERO, AX
	JMP*	AX
	 */
